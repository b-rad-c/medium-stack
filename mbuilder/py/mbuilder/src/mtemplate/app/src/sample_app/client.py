...
class SampClient(MStackClient):
    ...
    # for :: {% for model in models.with_endpoint %} :: {"sample_item": "model.snake_case", "sample item": "model.lower_case", "SampleItem": "model.pascal_case", "sample-item": "model.kebab_case", "sample-api": "api_prefix"}
    # sample item #

    def create_sample_item(self, creator: SampleItemCreator) -> SampleItem:
        data = self._post('sample-api/sample-item', json=creator.model_dump())
        return SampleItem(**data)
    
    def read_sample_item(self, id:str = None, cid:str = None) -> SampleItem:
        url = self._model_id_type_url('sample-api/sample-item', id, cid)
        data = self._get(url)
        return SampleItem(**data)
    
    def delete_sample_item(self, id:str = None, cid:str = None) -> None:
        self._delete(self._model_id_type_url('sample-api/sample-item', id, cid))

    def list_sample_items(self, offset:int=0, size:int=50) -> List[SampleItem]:
        data = self._get('sample-api/sample-item', params={'offset': offset, 'size': size})
